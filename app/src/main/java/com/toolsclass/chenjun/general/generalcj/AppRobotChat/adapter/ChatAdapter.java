...
import com.toolsclass.chenjun.general.generalcj.AppRobotChat.activity.WebActivity;
import com.toolsclass.chenjun.general.generalcj.AppRobotChat.bean.Answer;
import com.toolsclass.chenjun.general.generalcj.AppRobotChat.bean.Cook;
import com.toolsclass.chenjun.general.generalcj.AppRobotChat.bean.Msg;
import com.toolsclass.chenjun.general.generalcj.AppRobotChat.bean.News;
import com.toolsclass.chenjun.general.generalcj.AppRobotChat.config.Const;
import com.toolsclass.chenjun.general.generalcj.AppRobotChat.listener.RecordPlayClickListener;
...
import com.toolsclass.chenjun.general.generalcj.AppRobotChat.view.CircleImageView;
...
public class ChatAdapter extends BaseListAdapter<Msg> {
    ...
    private View createViewByType(Msg msg, int position) {
        switch (msg.getType()) {
            case Const.MSG_TYPE_TEXT://文本
                return getItemViewType(position) == TYPE_RECEIVER_TXT ? createView(R.layout.robot_item_chat_text_rece) : createView(R.layout.robot_item_chat_text_sent);
            case Const.MSG_TYPE_IMG://图片
                return getItemViewType(position) == TYPE_RECEIVER_IMAGE ? createView(R.layout.robot_item_chat_image_rece) : createView(R.layout.robot_item_chat_image_sent);
            case Const.MSG_TYPE_LOCATION://位置
                return getItemViewType(position) == TYPE_RECEIVER_LOCATION ? createView(R.layout.robot_item_chat_location_rece) : createView(R.layout.robot_item_chat_location_sent);
            case Const.MSG_TYPE_VOICE://语音
                return getItemViewType(position) == TYPE_RECEIVER_VOICE ? createView(R.layout.robot_item_chat_voice_rece) : createView(R.layout.robot_item_chat_voice_sent);
            case Const.MSG_TYPE_MUSIC://音乐
                return createView(R.layout.robot_item_chat_music_rece);
            case Const.MSG_TYPE_LIST://列表
                return createView(R.layout.robot_item_chat_news_rece);
            default:
                return null;
        }
    }
    ...
    public View bindView(final int position, View convertView, ViewGroup parent) {
        ...
        switch (msg.getType()) {
            case Const.MSG_TYPE_TEXT://文本
                tv_text.setText(ExpressionUtil.prase(mContext, tv_text, msg.getContent() == null ? "[数据异常]" : msg.getContent()));
        ...
        switch (answer.getCode()) {
            case "302000"://新闻
                ...
                if (TextUtils.isEmpty(news.getIcon())) {
                    iv_news_top_img.setImageResource(R.drawable.robot_splash_screen_b);
                }else{
    ...
    View createNewsView(final News news){
        View view =mInflater.inflate(R.layout.robot_item_list,null);
        ImageView iv= (ImageView) view.findViewById(R.id.iv);
        TextView tv= (TextView) view.findViewById(R.id.tv);
        if (TextUtils.isEmpty(news.getIcon())) {
            iv.setImageResource(R.drawable.robot_splash_screen_b);
        }else{
