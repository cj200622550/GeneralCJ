...
import com.toolsclass.chenjun.general.generalcj.AppMedia.model.ArtistInfo;
...
public class ArtistInfoActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.media_activity_artist_info);
    ...
    private void setData(ArtistInfo artistInfo) {
        ...
        if (!TextUtils.isEmpty(avatarUri)) {
            ...
            ivAvatar.setImageResource(R.drawable.media_default_artist);
            ivAvatar.setScaleType(ImageView.ScaleType.FIT_START);
            DisplayImageOptions options = new DisplayImageOptions.Builder()
                    .showStubImage(R.drawable.media_default_artist)
                    .showImageForEmptyUri(R.drawable.media_default_artist)
                    .showImageOnFail(R.drawable.media_default_artist)
                    .cacheInMemory(true)
                    .cacheOnDisk(true)
        ...
        if (!TextUtils.isEmpty(name)) {
            setTitle(name);
            TextView tvName = (TextView) LayoutInflater.from(this).inflate(R.layout.media_item_artist_info, llArtistInfoContainer, false);
            tvName.setText(getString(R.string.media_artist_info_name, name));
            llArtistInfoContainer.addView(tvName);
        }
        if (!TextUtils.isEmpty(country)) {
            TextView tvCountry = (TextView) LayoutInflater.from(this).inflate(R.layout.media_item_artist_info, llArtistInfoContainer, false);
            tvCountry.setText(getString(R.string.media_artist_info_country, country));
            llArtistInfoContainer.addView(tvCountry);
        }
        if (!TextUtils.isEmpty(constellation) && !constellation.equals("未知")) {
            TextView tvConstellation = (TextView) LayoutInflater.from(this).inflate(R.layout.media_item_artist_info, llArtistInfoContainer, false);
            tvConstellation.setText(getString(R.string.media_artist_info_constellation, constellation));
            llArtistInfoContainer.addView(tvConstellation);
        }
        if (stature != 0f) {
            TextView tvStature = (TextView) LayoutInflater.from(this).inflate(R.layout.media_item_artist_info, llArtistInfoContainer, false);
            tvStature.setText(getString(R.string.media_artist_info_stature, String.valueOf(stature)));
            llArtistInfoContainer.addView(tvStature);
        }
        if (weight != 0f) {
            TextView tvWeight = (TextView) LayoutInflater.from(this).inflate(R.layout.media_item_artist_info, llArtistInfoContainer, false);
            tvWeight.setText(getString(R.string.media_artist_info_weight, String.valueOf(weight)));
            llArtistInfoContainer.addView(tvWeight);
        }
        if (!TextUtils.isEmpty(birth) && !birth.equals("0000-00-00")) {
            TextView tvBirth = (TextView) LayoutInflater.from(this).inflate(R.layout.media_item_artist_info, llArtistInfoContainer, false);
            tvBirth.setText(getString(R.string.media_artist_info_birth, birth));
            llArtistInfoContainer.addView(tvBirth);
        }
        if (!TextUtils.isEmpty(intro)) {
            TextView tvIntro = (TextView) LayoutInflater.from(this).inflate(R.layout.media_item_artist_info, llArtistInfoContainer, false);
            tvIntro.setText(getString(R.string.media_artist_info_intro, intro));
            llArtistInfoContainer.addView(tvIntro);
        }
        if (!TextUtils.isEmpty(url)) {
            TextView tvUrl = (TextView) LayoutInflater.from(this).inflate(R.layout.media_item_artist_info, llArtistInfoContainer, false);
        ...
        if (llArtistInfoContainer.getChildCount() == 0) {
            ViewUtils.changeViewState(svArtistInfo, llLoading, llLoadFail, LoadStateEnum.LOAD_FAIL);
            ((TextView) llLoadFail.findViewById(R.id.tv_load_fail_text)).setText(R.string.media_artist_info_empty);
        }
    }
}
